...
class view_utilites():
    ...
    def button_photo_placeholder(self,path,div_name,side_x_photo,side_y_photo,position_x,position_y):
        logo_directory = os.path.join(self.script_dir, path)
        logo_home = CTkImage(light_image=Image.open(logo_directory), size=(side_x_photo,side_y_photo))
        logo_placeholder_home = CTkButton(div_name,image=logo_home,fg_color='transparent',text='',
                                          hover_color='#926565',height=side_y_photo,width=side_x_photo)
        logo_placeholder_home.place(x=position_x , y=position_y)
